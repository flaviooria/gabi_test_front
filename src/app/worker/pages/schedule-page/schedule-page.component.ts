...
export class SchedulePageComponent implements OnInit {
  ...
  private loadData(workerId: string): void {
    this.isLoading = true;
    this.workerService.getWorkerHorario(workerId).subscribe({
      next: (schedule) => {
  ...
  private generateMonthCalendar(year: number, month: number): CalendarMonth {
    ...
    while (dayCounter <= daysInMonth) {
      const servicesOnDay = this.schedule?.services.filter((service: Service) => {
